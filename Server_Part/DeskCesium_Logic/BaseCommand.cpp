...
void CBaseCommand::logic_insert_side_scan_source_data(const CMessage_Source& source, std::shared_ptr<CMessage_Packet> recv_packet, std::shared_ptr<CMessage_Packet> send_packet)
{
    ...
    for (uint32_t i = 0; i < size; i++)
    {
		...
		sprintf(sql, "REPLACE INTO t_source_data_side_scan VALUES('%s', '%s', %f, %f, %f, '%s', %f, %f, %f, '%s', %f, %f, '%s', %f, %d, '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s', '%s', 0)", 
				...
				source.horizontal_range_value(), source.height_from_bottom(), source.r_theat(), source.side_scan_image_name().data(), source.along_track(), 
				source.across_track(), source.remarks().data(), source.suppose_size(), source.priority(), source.verify_auv_sss_image_paths().data(), source.verify_image_paths().data(), 
